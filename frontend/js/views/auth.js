...
export function renderAuth(root, { onLogin }) {
  ...
  const page = el("div", { class: "auth-page" },
    el("div", { class: "auth-pitch" },
      ...
      el("p", { class: "auth-eyebrow" }, "A West African savings tradition, brought to the US"),
      el("h1", { class: "auth-headline" }, "Save as a group.", el("br"), "Take turns getting the pot."),
      el("p", { class: "auth-sub" },
        "For generations, West African communities have saved with a simple system — ajo — ",
        "where a few people who trust each other pool their money and take turns collecting it. ",
        "Ajo brings that idea to the US. Everyone puts in the same amount each week, and each ",
        "week one member takes the whole pot. It rotates until everyone has had a turn. A simple, ",
        "interest-free way to save for something big — no bank, no loan, no interest."
      ),
